...
1. **Read Transcript Files**
   ...
   a. **Determine the parent directory** of the team-management/ directory in which the task file is stored
   b. **List the SUBDIRECTORIES** of `[parent directory]/.claude/state/context-refinement/`. Transcript chunks are staged one level deep, in a per-invocation key directory `.claude/state/context-refinement/<key>/` — the direct children here are DIRECTORIES, not files; parallel invocations may leave several.
   c. **Select the key directory for THIS invocation** (the only one, or the most recently modified if several) and **read every file inside it** (`.claude/state/context-refinement/<key>/*`).
