...
4. **REMOVE irrelevant content**:
   ...
   - Do NOT edit Success Criteria text (see the wording rule below) — only their checkbox state may change
...
1. **Determine the parent directory** of the team-management/ directory in which the task file is stored
2. **List the SUBDIRECTORIES** of `[parent directory]/.claude/state/logging/`. Transcript chunks are staged one level deep, in a per-invocation key directory `.claude/state/logging/<key>/` — so the direct children here are DIRECTORIES, not files. Parallel logging invocations may leave several key directories side by side.
3. **Select the key directory for THIS invocation**: if there is exactly one, use it; if there are several (parallel runs), use the most recently modified one.
4. **Read every file INSIDE that key directory** (`.claude/state/logging/<key>/*`) — those files are the transcript chunks.
...
5. **Scope of Updates**
   ...
   - Update Success Criteria **checkbox state only** — never the criterion wording (see the wording rule)
...
**Success Criteria — Checkbox State Only (NEVER reword the criterion text):**

You MAY flip a checkbox (`[ ]`↔`[x]`) as work completes. You MUST NOT reword, shorten, paraphrase, or "tidy" the criterion text — the `spec-compliance-reviewer` audits the diff against the criteria text **verbatim**, so weakening the wording silently weakens the spec audit.

Before:
```
- [ ] Authentication works with proper JWT token validation and session management including Redis caching
...
- Change DAIC mode or session/workflow state
