...
2. **Explore and ask** with the Question Discipline procedure: draft a `TodoWrite` backlog of clarifying questions across eight dimensions (purpose, scope, affected modules, expected behaviour, edge cases / failure modes, non-functional requirements, terminology / data definitions, integration points), prioritize by **Impact × Uncertainty** with a **max-5 widget-questions cap** (remainder closed as reasonable defaults recorded in `## User Notes` — the cap limits questions, not the zero-ambiguity end state), then ask via `AskUserQuestion` **strictly one question per call, recommendation-first** (a `Recommendation: <option> — <reasoning>` prose line before every option-based widget, recommended option listed first; free-text questions exempt) with the field-content discipline (the `question` carries 1-2 sentences of standalone context; each option `description` states the trade-off, not just the label).
3. **Propose 2-3 distinct approaches with trade-offs** — never present a single approach and proceed.
4. **Compose the Implementation Plan with the No-Placeholders rule**: exact paths and line ranges, full snippets, exact commands with expected output. `TBD`/`TODO`/"similar to Task N" are forbidden inside the plan. **One sanctioned draft-time exception**: an unresolved point may be marked inline with a bracketed `NEEDS CLARIFICATION: <question>` marker (max 3 per file, priority scope > security > UX > technical details; reasonable defaults are picked, not asked). Every marker must be resolved before delivery — `_validate_task_file_structure` in `protocol_engine.py` rejects a delivered task file whose body still matches `\[NEEDS\s+CLARIFICATION` (case-insensitive), on both delivery paths in all protocols.
5. **Plan self-review** before advancing: spec-coverage, **ID coverage** (every `SC-N` criterion referenced by at least one plan step's `[SC-N]` tag), placeholder scan, type-consistency.
6. **Numbered spec convention** (spec-kit-inspired): Success Criteria carry stable IDs (`- [ ] SC-1: ...`; never renumbered, append-only) and Implementation Plan steps are numbered with coverage tags (`- [ ] T1 [SC-1]: ...`). Convention only — the engine does not validate the ID format; it is defined in `TEMPLATE.md`, `task-investigation.md` §6, and `task-creation.md`.
7. **Context Files — hybrid rule**: a DIRECTORY task (task file at `team-management/tasks/<task>/README.md`) or a frontmatter `modules` list with >3 entries → dispatch the `context-gathering` agent AFTER writing the draft task file (write-file-first) and BEFORE `protocol_advance`, passing the task file path (the agent requires an existing file and writes the Context Manifest into it); otherwise fill `## Context Files` manually.
8. **Alignment before advance**: present the summary, wait for **explicit** user agreement (silence ≠ agreement), read `TEMPLATE.md`, then deliver the task file. **Preferred for substantial files — write-file-first**: write the markdown to `team-management/tasks/<name>.md` (DIRECTORY task: `<name>/README.md` — the engine resolves both, whitelisted in discussion mode) and advance with an **empty** `task_content` — the engine re-validates the on-disk file with strict parity and never overwrites it (see [Protocol Engine](pages/subsystems/protocol-engine.md)). Small files may pass full `task_content` inline. Large object-typed MCP args are unreliable (a multi-KB `task_content` can arrive empty / `__unparsedToolInput` above ~2KB), so emit the call **bare** and keep `args` small.
...
Warning enforcement (Section 5): the mode is read via the `config_code_review_enforcement` MCP tool as the mandatory first step — a direct `team-management/config.json` read is an error-only fallback (failed call or `success: false`), limited to the single `enforce_warnings` key. In **strict** mode (`enforce_warnings=true`) the LLM fixes all warnings automatically and re-reviews until clean; in **relaxed** mode warnings are documented but don't block. Critical issues always block.
...
- **The clarification-marker gate scans the whole body and is one-way sticky.** A verbatim canonical marker anywhere in a delivered task file (work log quote included) blocks delivery AND any later re-investigate advance of that file. When *documenting* the marker, defang it (ellipsis/hyphen between the words); the `\s+` in the gate's pattern means regex-source spellings like `\[NEEDS\s+CLARIFICATION` do NOT self-match (literal `\s` is not whitespace) — verified empirically, and test sources build the literal by string concatenation.
